...
Try it:
```
curl -X POST localhost:8080/v1/jobs -H 'Content-Type: application/json' -H 'X-Install-ID: dev-1' \
...
Tests: `.venv/bin/pytest`. Deploy: see `fly.toml`.

## How each platform is fetched
| | |
|---|---|
| TikTok | Own mobile page JSON (iPhone UA) → `playAddr` / `imagePost.images[]`. Free. |
